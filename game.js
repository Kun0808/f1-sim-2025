...
function retire() {
  ...
  const totalRaces = gameState.raceResults.length;
  ...
  const dnfs = gameState.raceResults.filter(r => r.dnf).length;
  const bestFinish = gameState.raceResults.reduce((best, r) => {
    if (r.dnf) return best;
    return r.position < best ? r.position : best;
  }, 99);
  const avgPos = totalRaces > 0
    ? (gameState.raceResults.reduce((sum, r) => sum + (r.dnf ? 20 : r.position), 0) / totalRaces).toFixed(1)
    : '-';
