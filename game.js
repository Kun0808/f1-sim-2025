...
function finishQuickSim(simResults) {
  ...
  // Calculate projected income for quick sim
  const qWins = gameState.raceResults.filter(r => !r.dnf && r.position === 1).length;
  const qP2 = gameState.raceResults.filter(r => !r.dnf && r.position === 2).length;
  const qP3 = gameState.raceResults.filter(r => !r.dnf && r.position === 3).length;
  let qBonus = qWins * 2.0 + qP2 * 1.0 + qP3 * 0.5;
  if (isChampion) qBonus += 10;
  if (quickPlayerTeamStanding) {
    if (quickPlayerTeamStanding.position === 1) qBonus += 5;
    else if (quickPlayerTeamStanding.position === 2) qBonus += 3;
    else if (quickPlayerTeamStanding.position === 3) qBonus += 2;
  }
  const qSponsor = gameState.sponsor ? (gameState.sponsor.salary || 0) : 0;
  const qTotal = (gameState.salary || 5) + qSponsor + qBonus;
  ...
  document.getElementById('hub-screen').innerHTML = `
    ...
    <div class="card" style="padding:14px;margin-top:12px;border-color:var(--green);">
      ...
      <div style="display:grid;gap:6px;font-size:0.85rem;">
        ...
        <div style="display:flex;justify-content:space-between;">
          ...
          <span style="color:var(--green);font-weight:600;">$${qSponsor.toFixed(1)}M</span>
        </div>` : ''}
        <div style="display:flex;justify-content:space-between;">
          <span style="color:var(--text-muted);">🏆 比赛奖金</span>
          <span style="color:var(--green);font-weight:600;">$${qBonus.toFixed(1)}M</span>
        </div>
        ${qBonus > 0 ? `
        <div style="font-size:0.75rem;color:var(--text-muted);padding-left:12px;">
          ${qWins > 0 ? `分站冠军 ×${qWins} = $${(qWins * 2.0).toFixed(1)}M<br>` : ''}
          ${qP2 > 0 ? `亚军 ×${qP2} = $${(qP2 * 1.0).toFixed(1)}M<br>` : ''}
          ${qP3 > 0 ? `季军 ×${qP3} = $${(qP3 * 0.5).toFixed(1)}M<br>` : ''}
          ${isChampion ? `世界冠军奖金 = $10.0M<br>` : ''}
          ${quickPlayerTeamStanding && quickPlayerTeamStanding.position <= 3 ? `车队排名 #${quickPlayerTeamStanding.position} 奖金 = $${quickPlayerTeamStanding.position === 1 ? '5.0' : quickPlayerTeamStanding.position === 2 ? '3.0' : '2.0'}M<br>` : ''}
        </div>` : ''}
        <div style="display:flex;justify-content:space-between;padding-top:6px;border-top:1px solid var(--border);">
          ...
          <span style="color:var(--green);font-weight:700;font-family:'Orbitron';">$${qTotal.toFixed(1)}M</span>
        </div>
      </div>
    </div>
...
function endSeason() {
  ...
  // Show projected income breakdown
  ...
  if (playerTeamStanding) {
    if (playerTeamStanding.position === 1) projectedBonus += 5;
    else if (playerTeamStanding.position === 2) projectedBonus += 3;
    else if (playerTeamStanding.position === 3) projectedBonus += 2;
  }
  ...
  const incomeBtn = document.querySelector('#season-end-screen .btn-primary');
  if (incomeBtn) {
    ...
    incomeDiv.innerHTML = `
      ...
      <div style="display:grid;gap:6px;font-size:0.85rem;">
        ...
        <div style="font-size:0.75rem;color:var(--text-muted);padding-left:12px;">
          ...
          ${isChampion ? `世界冠军奖金 = $10.0M<br>` : ''}
          ${playerTeamStanding && playerTeamStanding.position <= 3 ? `车队排名 #${playerTeamStanding.position} 奖金 = $${playerTeamStanding.position === 1 ? '5.0' : playerTeamStanding.position === 2 ? '3.0' : '2.0'}M<br>` : ''}
        </div>` : ''}
        <div style="display:flex;justify-content:space-between;padding-top:6px;border-top:1px solid var(--border);">
          <span style="font-weight:600;">总计</span>
    ...
    `;
    incomeBtn.parentElement.insertBefore(incomeDiv, incomeBtn);
  }
